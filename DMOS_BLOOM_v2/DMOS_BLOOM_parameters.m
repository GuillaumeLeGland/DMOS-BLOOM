...
param.Kn   = [0.5; 0.5; 0.5; 0.5]; % Half-saturation constant for DIN (µM)
param.mp   = [0.05; 0.05; 0.05; 0.05]; % Linear mortality rate (d-1)

% HETEROTROPHIC BACTERIA
param.mub0 = 5.0;  % Maximum uptake rate (d-1)
param.Kon  = 5.0;  % Half-saturation constant for ON (µM)
%param.Aon  = param.mub0 / param.Kon; % Affinity for ON (d-1 µM-1)
...
param.Kz    = 4.0 * ones(5,1);  % Half-saturation constant for food (µM-N)
param.Kz(1) = 2.0;    
...
param.mz2   = 0.5 * ones(5,1);  % Quadratic mortality rate (µM-1 d-1)
...
% Initial nitrate concentration (µM)
param.DIN0  = 0.27;
param.ON0   = 1.5;  % Around 10.0 in observations. We will suppose 15% is labile.

% Initial concentration of phytoplankton species, in µM-N (Le Gland, 29/11/2021)
param.pico0 = 0.45;
