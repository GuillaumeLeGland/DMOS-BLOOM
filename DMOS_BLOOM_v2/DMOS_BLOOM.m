...
[keyInvPlankton,keyInvSulfur,keyObsTolerance,keyGrowthDMSP,keyGrowthDMS,keyVarDMSUptake,keyTemp,keyPAR] = DMOS_BLOOM_keys(); 
...
[day,Ehux_obs,nano_obs,pico_obs,Syn_obs,bac_obs,EhV_obs,MpV_obs,cil_obs,thr_obs,IEC_obs,...
 NO3_obs,Chl_obs,DMS_obs,DMSPd_raw,DMSPd_obs,DMSPp_obs,PON_obs] = DMOS_BLOOM_load_data();
...
% PON_obs has been added during revision
raw_obs_plankton = cat(3,pico_obs,nano_obs,Ehux_obs,cil_obs,bac_obs,NO3_obs,PON_obs);

offset = 2*nanmean(NO3_obs(8,:)); % Account for NH4 and NO2 
raw_obs_plankton(:,:,6)   = raw_obs_plankton(:,:,6) + offset;
weight_obs_plankton = [1,1,1,0,2,1,0]; % Weight of each variable in the NPZD cost function
...
if strcmp(keyInvPlankton,'yes')
    ...
    X = [0.666 0.671 0.603 0.652 3.18 7.91 0.593 4.38 2.42 2.67 1.81 1.83 2.31 2.29 2.69 2.46 3.27 1.08 0.957 0.599 3.6e-008 0.49];
end
...
if strcmp(param0.outputFluxes,'yes')
    ...
    FLUXES.graz_to_DMS    = zeros(nphy,nsteps,nsim);
...
color = [0.5 0.5 0.5; 0.5 0.5 0.5; 0.5 0.5 0.5; 1 0 0; 0.5 0.5 0.5; 0.5 0.5 0.5; 0 0 1; 0 0 0];
color4 = [0 0 0; 0.5 0.5 0.5; 1 0 0; 0 0 1];
...
fignum = 430;
DMOS_BLOOM_plot_PAR(param0.PAR,Chl_obs,day_obs,ndays,color,fignum)
