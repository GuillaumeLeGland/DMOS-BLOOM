...
param.Kz(1) = 2.0;    
%param.Az    = param.muz0 ./ param.Kz; % Affinity for food (d-1 (然-N)-1)
...
param.muDMS0_coMet = 0.15; % Maximum uptake rate by co-metabolizers (?)
param.muDMS0_spec  = 6.0;  % Maximum uptake rate by specialists (?)
param.KDMS         = 8.0;  % Half-saturation constant (nM)

% Dissolved DMSP uptake
if Scenario >= 3
    param.muDMSPd0 = 0.6;  % Maximum DMSPd uptake rate by bacteria (?)
else
    param.muDMSPd0 = 0.9;
end
...
if Scenario >= 3
    param.muDMSPd0_phy = [0.06; 0.0; 0.0; 0.0]; % Scenarios S3 and S4
else
...
param.leak_DMS_frac  = [0.0; 0.06; 0.06; 0.18];

% Extra DMSP and DMS leakage during infection in scenario S4
if Scenario >= 4
    param.leak_inf_fac = 2.8; % 280% increase
else
